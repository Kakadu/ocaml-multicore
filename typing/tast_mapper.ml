...
type mapper =
  {
    ...
    type_declarations: mapper -> type_declaration list ->
...
let structure_item sub {str_desc; str_loc; str_env} =
  ...
  let str_desc =
    ...
    | Tstr_type list ->
        let list = sub.type_declarations sub list in
        Tstr_type list
    ...
    | Tstr_class xs ->
        Tstr_class (List.map (fun (decl, xs, flg) -> (sub.class_declaration sub decl, xs, flg)) xs)
    | Tstr_class_type list ->
        Tstr_class_type
          (List.map (tuple3 id id (sub.class_type_declaration sub)) list)
    (* | Tstr_class_type (ident, loc, ctdecl) ->
        Tstr_class_type (ident, loc, sub.class_type_declaration sub ctdecl) *)
    | Tstr_include incl ->
...
(* records as constructor arguments are not yet in 4.02.2 *)
let constructor_args sub = List.map (sub.typ sub)
...
let type_declarations sub list =
  List.map (sub.type_declaration sub) list
...
let expr sub x =
  ...
  let exp_desc =
    ...
    | Texp_match (exp, cases1, cases2, cases3, p) ->
        let exp = sub.expr sub exp in
        let cases1 = sub.cases sub cases1 in
        let cases2 = sub.cases sub cases2 in
        let cases3 = sub.cases sub cases3 in
        Texp_match (exp, cases1, cases2, cases3, p)
    | Texp_try (exp, cases, cases2) ->
        Texp_try (
          sub.expr sub exp,
          sub.cases sub cases,
          sub.cases sub cases2
        )
    | Texp_tuple list ->
    ...
    | Texp_record (xs, e_opt) ->
        let e_opt = match e_opt with
        | Some e -> Some (sub.expr sub e)
        | None -> None
        in
        let xs = List.map (fun (loc, desc, expr) -> (loc, desc, sub.expr sub expr)) xs in
        Texp_record (xs, e_opt)
...
let module_coercion sub = function
  ...
  | Tcoerce_primitive pc -> Tcoerce_primitive pc
